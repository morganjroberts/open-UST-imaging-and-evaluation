...
for tdx = 1:Ntdx
    ...
    for rdx = 1:Nrdx
        if mask(tdx, rdx)
            ...
            i_fm            = minimumAIC(trace_n(:));
            i_fms(tdx, rdx) = i_fm;

            if options.ExtraPlot && tdx == 1 && rdx == 101
                minimumAIC(trace_n(:), Plot=true);
            end
            ...
            if options.ExtraPlot && tdx == 1 && rdx == 178
                ...
                xlabel('Time [samples]');
                ylabel('Voltage [au]');        
                title('Processed Trace, tx = 1, rx = 178');
    ...
    if options.ExtraPlot && tdx == 1
        ...
        title('First arrival capture, tx = 1');
        drawnow
